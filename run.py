...
def reg():
    ...
    if request.method == 'POST':
        ...
        for t in [(user_name, password, email, telephone), ]:
            sql_ins = "insert into sys_users(user_name,password,email,telephone) values (?,?,?,?)"
...
@app.route('/existUser', methods=['GET', 'POST'])
def existUser():
    if request.method == "POST":
        error = ""
        cx = sqlite3.connect(JDB_NAME)
